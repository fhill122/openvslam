...
bool keyframe_inserter::new_keyframe_is_needed(const data::MultiFrame& curr_frm, const unsigned int num_tracked_lms,
    ...
    const auto num_reliable_lms = ref_keyfrm.getNumTrackedLm(min_obs_thr);
...
std::shared_ptr<data::MultiKeyframe> keyframe_inserter::insert_new_keyframe(data::MultiFrame& curr_frm) {
    ...
    auto keyfrm = data::MultiKeyframe::Create(curr_frm, map_db_);
    ...
    // todo [ivan] consider how multikeyframe should do this
    //  - should we do this when triangulating temporally?
    /*
...
void keyframe_inserter::queue_keyframe(const std::shared_ptr<data::MultiKeyframe>& keyfrm) {
